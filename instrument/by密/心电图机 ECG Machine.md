...
- **Frequency Modulation (调频调制) Chip 芯片** - Converts voltage to frequency (V-F) for transmission through isolation barrier
...
- **Optical Isolation (光隔离)** - Creates electrical safety barrier between patient-connected circuitry and measurement electronics
	...
	和12V电压共同加载到发光二极管
- **Demodulation (解调) Chip** - Converts frequency back to voltage (F-V) after isolation
	变低频
...
- **[[Low-pass Filter 低通滤波]]** - Removes high-frequency noise while preserving the important ECG waveform features.
	uses [[capacitors]] in an [[RC circuit  RC电路]] configuration
